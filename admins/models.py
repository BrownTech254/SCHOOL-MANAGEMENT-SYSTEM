...
class Admin(models.Model):
	user = models.OneToOneField(CustomeUser, on_delete=models.CASCADE, related_name='admin')
...
class Admin_Personal_Data(models.Model):
	admin = models.OneToOneField(Admin, on_delete=models.CASCADE, related_name='personal_data')
	first_name = models.CharField(max_length=255, null=True,blank=True)
	last_name = models.CharField(max_length=255, null=True,blank=True)
	...
	def save(self, *args, **kwargs):
		if self.first_name is None:
			self.first_name = self.admin.user.first_name
		if self.last_name is None:
			self.last_name = self.admin.user.last_name
		if self.email is None:
			self.email = self.admin.user.email

		super().save(*args, **kwargs)

class Admin_Family_Information(models.Model):
	admin = models.OneToOneField(Admin, on_delete=models.CASCADE, related_name='family')
...
class Admin_Residence(models.Model):
	admin = models.OneToOneField(Admin, on_delete=models.CASCADE, related_name='residence')
...
class Admin_Emergency_Contact_Persons(models.Model):
	admin = models.OneToOneField(Admin, on_delete=models.CASCADE, related_name='emergency_contacts')
...
class Academic_Background(models.Model):
	admin = models.OneToOneField(Admin, on_delete=models.CASCADE, related_name='academic_data')
